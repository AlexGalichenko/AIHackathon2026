...
test.describe("Part Categories — CRUD", () => {
  ...
  test("TC-PCAT delete non-existent category returns 404", async ({ apiRequest }) => {
    const resp = await apiRequest.delete(`/api/part/category/${MISSING_ID}/`);
    expect(resp.status()).toBe(404);
  });

  test("TC-PCAT full update category with PUT returns 200", async ({ apiRequest }) => {
    const cat = await createCategory(apiRequest);
...
test.describe("Part Categories — Auth", () => {
  ...
  test("TC-PCAT unauthenticated POST returns 401", async ({ request }) => {
    const baseUrl = process.env.BASE_URL ?? "http://localhost:8000";
...
test.describe("Part Categories — Pagination", () => {
  test("TC-PCAT limit=1 returns at most one result", async ({ apiRequest }) => {
  ...
  test("TC-PCAT offset beyond count returns empty list", async ({ apiRequest }) => {
...
test.describe("Part Categories — Ordering", () => {
  ...
  for (const ordering of VALID_ORDERINGS) {
    test(`TC-PCAT ordering=${ordering} returns 200`, async ({ apiRequest }) => {
      const resp = await apiRequest.get("/api/part/category/", { params: { limit: 5, ordering } });
      expect(resp.status()).toBe(200);
    });
  }

  test("TC-PCAT ordering by invalid field returns 200 (API silently ignores unknown ordering)", async ({ apiRequest }) => {
...
test.describe("Part Categories — Search", () => {
  ...
  test("TC-PCAT search by created category name returns result", async ({ apiRequest }) => {
...
test.describe("Part Categories — Field Validation", () => {
  ...
  test("TC-PCAT description over max length (251 chars) returns 400", async ({ apiRequest }) => {
    const resp = await apiRequest.post("/api/part/category/", {
  ...
  test("TC-PCAT parent=null accepted when nullable", async ({ apiRequest }) => {
  ...
  test("TC-PCAT read-only field pk in POST body is ignored", async ({ apiRequest }) => {
...
test.describe("Part Categories — Relational Integrity", () => {
  ...
  test("TC-PCAT default_location set to non-existent ID returns 400", async ({ apiRequest }) => {
    const resp = await apiRequest.post("/api/part/category/", {
